...
def parser(proto, shape=(400, 400, 3)):
    ...
    parsed_example = tf.io.parse_single_example(
        serialized=proto, features=data)
    image_raw = parsed_example['image']
    image = tf.io.decode_raw(image_raw, tf.float32)
    image = tf.reshape(image, shape=shape)
    # Since input image==label making a copy (might be inefficient)
    return image, image
...
def from_tfrecords(records_globs, split, batch_size):
    ...
    dataset = tf.data.TFRecordDataset(records_globs)
    ...
    # image = tf.reshape(image, shape=[-1, 400, 400, 3])
